...
def daily_digest(
    ...
    # arXiv Papers
    arxiv_papers = _fetch_arxiv_papers(arxiv_limit, dry=dry)
    ...
    # Generate insights and summaries using DeepSeek
...
def _fetch_arxiv_papers(limit: int, dry: bool = True) -> list[dict[str, Any]]:
    """Fetch recent arXiv papers with summaries."""
    try:
        ...
        papers = []
        for _, row in top_papers.iterrows():
            # Only summarize if not in dry mode
            if dry:
