...
@retry(
    ...
    wait=wait_exponential(
        multiplier=1, min=1, max=min(64, ARXIV_ZERO_RESULTS_MAX_WAIT_TIME)
    ),
    before_sleep=_log_retry_attempt,
    reraise=True,
)
def _fetch_from_arxiv(
    after_date: datetime, before_date: datetime, max_results: int = 1000
) -> list[dict[str, Any]]:
    category_query = (
    ...
    while True:
        ...
        try:
            ...
            logger.info(
                f"Fetched {len(entries)} entries from Arxiv API starting at {start}"
            )
            ...
            for entry in entries:
                ...
                after_dt = datetime.fromisoformat(
                    after_date.isoformat().replace("Z", "+00:00")
                )
                before_dt = datetime.fromisoformat(
                    before_date.isoformat().replace("Z", "+00:00")
                )
...
def _fetch_from_arxiv_api(
    base_url: str, query: dict[str, Any]
    ...
    for _ in range(ARXIV_NUM_RETRIES):
        try:
            ...
            logger.error(
                "Could not get results from arxiv.",
                exc_info=True,
                extra={"exception": str(e)},
            )
    ...
    logger.critical(
        f"Could not get results from arxiv after {ARXIV_NUM_RETRIES} trials"
    )
    raise FatalError(
        f"Could not get results from arxiv after {ARXIV_NUM_RETRIES} trials"
    )
